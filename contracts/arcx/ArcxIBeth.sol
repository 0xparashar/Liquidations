...
contract ArcxIBeth is IStructs {
    ...
    IarcxPool arcxPool = IarcxPool(address(0x81f16E6B58738DaAA4C5D493a240cCA49C240f04));
    ...
    IBeth iBeth = IBeth(address(0x67B66C99D3Eb37Fa76Aa3Ed1ff33E8e39F0b9c7A));

    IWeth weth = IWeth(address(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2));
    
    address owner;
    
    uint256 BASIS_POINT = 10000;
    ...
    function uniswapV2Call(address pair, uint256 amount0Out, uint256 amount1Out, bytes memory data) external {
        ...
        (uint256 amount, uint256 amountUSDC, uint256 position, uint256 minerPercent) = abi.decode(data, (uint256, uint256, uint256, uint256));
        ...
        stablex.approve(address(arcxPool), uint256(-1));
        arcxPool.operateAction(Operation.Liquidate, params);

        require(iBeth.balanceOf(address(this)) > 0, "!no collateral received");

        iBeth.withdraw(iBeth.balanceOf(address(this)));
        ...
        uint256 minerFee = address(this).balance.mul(minerPercent).div(BASIS_POINT);
    ...
    function run(uint256 posIndex, uint256 minerPercent) external {

        require(msg.sender == owner, "!owner");
        require(minerPercent < BASIS_POINT);
        ...
        bytes memory data = abi.encode(amount, amountIn, posIndex, minerPercent);
