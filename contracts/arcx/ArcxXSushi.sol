...
contract ArcxXSushi is IStructs {
    ...
    function uniswapV2Call(address pair, uint256 amount0Out, uint256 amount1Out, bytes memory data) external {
        ...
        uint256 minerFees = profit.mul(minerPercent).div(BASIS_POINT);
        
        if(minerFees > 0){
        
            _swapToGetWeth(minerFees);
            
            weth.withdraw(weth.balanceOf(address(this)));
            block.coinbase.transfer(address(this).balance);
            
            profit = profit.sub(minerFees);
    ...
    function _swapToGetWeth(uint _amountIn) internal {
